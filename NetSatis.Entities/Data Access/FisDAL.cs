...
namespace NetSatis.Entities.Data_Access
{
    public class FisDAL : EntityRepositoryBase<NetSatisContext, Fis, FisValidator>
    {
        ...
        public object ListelemelerTarih(NetSatisContext context, string fisTuru, string fisTuru2, string fisTuru3, string fisTuru4, string fisTuru5, string fisTuru6,string fisTuru7, DateTime baslangic, DateTime bitis)
        {
        ...
        public object FaturaYazdir(NetSatisContext context, string fisTuru, string fisTuru2, string fisTuru3, string fisTuru4, DateTime baslangic, DateTime bitis)
        {
            var tablo = context.Fisler.Where(c => (c.FisTuru == fisTuru || c.FisTuru == fisTuru2 || c.FisTuru == fisTuru3 || c.FisTuru == fisTuru4) && (c.Tarih >= baslangic && c.Tarih <= bitis)).GroupJoin(
                context.Fisler.Where(c => c.FisTuru == fisTuru || c.FisTuru == fisTuru2 || c.FisTuru == fisTuru3 || c.FisTuru == fisTuru4&& (c.Tarih >= baslangic && c.Tarih <= bitis)), c => c.CariId, c => c.CariId,
                (fisler, cariler) =>
                   new
                   {
                       fisler.Id,
